...
template<class real>
sqcu::CUDADenseGraphBFSearcher<real> *pyobjToCppObj(PyObject *obj) {
    npy_uint64 val = PyArrayScalar_VAL(obj, UInt64);
    return reinterpret_cast<sqcu::CUDADenseGraphBFSearcher<real>*>(val);
}
...
PyObject *dg_bf_searcher_create(PyObject *module, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "O", &dtype))
        return NULL;
    if (isFloat64(dtype))
        ext = (void*)new sqcu::CUDADenseGraphBFSearcher<double>();
    else if (isFloat32(dtype))
        ext = (void*)new sqcu::CUDADenseGraphBFSearcher<float>();
    else
...
PyObject *internal_dg_bf_searcher_get_x(PyObject *objExt) {
    sqaod::SizeType N;
    sqcu::CUDADenseGraphBFSearcher<real> *sol = pyobjToCppObj<real>(objExt);
