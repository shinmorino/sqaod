...
static PyObject *Cuda_DgSolverError;
namespace sq = sqaod;
...
template<class real>
sqcu::CUDADenseGraphAnnealer<real> *pyobjToCppObj(PyObject *obj) {
    npy_uint64 val = PyArrayScalar_VAL(obj, UInt64);
    return reinterpret_cast<sqcu::CUDADenseGraphAnnealer<real> *>(val);
}
...
PyObject *dg_annealer_create(PyObject *module, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "O", &dtype))
        return NULL;
    if (isFloat64(dtype))
        ext = (void*)new sqcu::CUDADenseGraphAnnealer<double>();
    else if (isFloat32(dtype))
        ext = (void*)new sqcu::CUDADenseGraphAnnealer<float>();
    else
        RAISE_INVALID_DTYPE(dtype, Cuda_DgSolverError);
...
PyObject *dg_annealer_set_problem(PyObject *module, PyObject *args) {
    ...
    TRY {
        if (isFloat64(dtype))
            ...
            RAISE_INVALID_DTYPE(dtype, Cuda_DgSolverError);
    } CATCH_ERROR_AND_RETURN(Cuda_DgSolverError);
...
void internal_dg_annealer_get_E(PyObject *objExt, PyObject *objE) {
    ...
    NpVector E(objE);
    sqcu::CUDADenseGraphAnnealer<real> *ext = pyobjToCppObj<real>(objExt);
    E.vec = ext->get_E();
}
...
PyObject *internal_dg_annealer_get_x(PyObject *objExt) {
    sqcu::CUDADenseGraphAnnealer<real> *ann = pyobjToCppObj<real>(objExt);
...
initcuda_dg_annealer(void) {
    ...
    Cuda_DgSolverError = PyErr_NewException(name, NULL, NULL);
    Py_INCREF(Cuda_DgSolverError);
    PyModule_AddObject(m, "error", Cuda_DgSolverError);
}
