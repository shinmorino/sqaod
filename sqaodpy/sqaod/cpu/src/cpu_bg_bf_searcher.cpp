...
template<class real>
using BipartiteGraphBFSearcher = sq::cpu::BipartiteGraphBFSearcher<real>;
...
static
PyMethodDef cpu_bg_bf_searcher_methods[] = {
...
extern "C"
PyMODINIT_FUNC
initcpu_bg_bf_searcher(void) {
    PyObject *m;
    
    m = Py_InitModule("cpu_bg_bf_searcher", cpu_bg_bf_searcher_methods);
    import_array();
    if (m == NULL)
        return;
    
    char name[] = "cpu_bg_searcher.error";
    Cpu_BgBfSearcherError = PyErr_NewException(name, NULL, NULL);
    Py_INCREF(Cpu_BgBfSearcherError);
    PyModule_AddObject(m, "error", Cpu_BgBfSearcherError);
}
