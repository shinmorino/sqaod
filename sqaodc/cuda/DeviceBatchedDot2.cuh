...
struct In2TypeDotPtrVec4 {
    ...
    typedef typename ValueTraits<Vout>::VectorType VecOut;
    ...
    __device__ __forceinline__
    VecOut operator[](sq::IdxType idx) const {
        VecIn0 x = d_x[idx];
        VecIn1 y = d_y[idx];
        VecOut mul4 = {
            (Vout)x.x * (Vout)y.x,
    ...
    __device__ __forceinline__
    VecOut operator[](const int2 &idx2) const {
        VecIn0 x = d_x[idx2.x];
        VecIn1 y = d_y[idx2.y];
        VecOut mul4 = {
            (Vout)x.x * (Vout)y.x,
...
template<class V>
struct iterator_traits<sqaod_cuda::InSumPtrVec4<V>> : sqaod_cuda::base_iterator_traits<V> { };

template<class Vout, class Vin0, class Vin1>
struct iterator_traits<sqaod_cuda::In2TypeDotPtrVec4<Vout, Vin0, Vin1>> : sqaod_cuda::base_iterator_traits<sqaod_cuda::ValueTraits<Vout>::VectorType> { };
